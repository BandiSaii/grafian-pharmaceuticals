...
export function CertificationsPage({ route, navigate }: CertificationsPageProps) {
  return (
    <div className="flex flex-col">
      ...
      {/* CERTIFICATION BADGES — premium cards with large badges and elegant framing */}
        ...
        <div className="container mx-auto px-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-7 max-w-5xl mx-auto">
            {certifications.map((c, idx) => (
              <motion.div
                ...
                whileHover={{ y: -4 }}
                className="group relative rounded-3xl bg-gradient-to-br from-grafian-blue-50/80 via-white to-grafian-blue-50/40 p-8 md:p-10 border border-grafian-blue-100 shadow-pharma hover:shadow-pharma-lg transition-all text-center overflow-hidden"
              >
                {/* Decorative blobs */}
                <div className="absolute -top-16 -right-16 h-40 w-40 rounded-full bg-grafian-blue-200/30 blur-3xl opacity-70 group-hover:opacity-100 transition-opacity" />
                <div className="absolute -bottom-16 -left-16 h-40 w-40 rounded-full bg-grafian-blue-100/40 blur-3xl opacity-70 group-hover:opacity-100 transition-opacity" />

                <div className="relative">
                  {/* Badge with soft halo */}
                  <div className="relative inline-flex items-center justify-center mb-6">
                    {/* Halo rings */}
                    <div className="absolute inset-0 rounded-full bg-grafian-blue-200/40 blur-2xl scale-150" />
                    <div className="absolute inset-0 rounded-full bg-white shadow-pharma-lg scale-110" />
                    {/* Badge image */}
                    <img
                      src={c.image}
                      alt={c.name}
                      className="relative h-40 w-40 md:h-44 md:w-44 object-contain drop-shadow-lg"
                    />
                  </div>
                  ...
                  <p className="text-sm md:text-[15px] text-slate-600 leading-relaxed max-w-md mx-auto">
                    {c.description}
                  </p>

                  <div className="mt-6 inline-flex items-center gap-1.5 rounded-full bg-gradient-to-r from-green-50 to-emerald-50 border border-green-200 px-4 py-1.5 text-xs font-semibold text-green-700">
          ...
          <div className="mt-14 flex flex-wrap justify-center items-center gap-3">
            {['WHO-GMP Certified', 'ISO 9001:2015 Certified', 'Made in India', 'Quality Assured'].map((t) => (
              <div
                key={t}
                className="inline-flex items-center gap-2 rounded-full bg-grafian-blue-50 border border-grafian-blue-100 px-5 py-2.5 text-[13px] font-semibold text-grafian-blue-deep"
              >
